...
# Data 
R [@R], a statistical programming language, is exclusively used in this paper. To clean and prepare the data, the libraries tidyverse [@tidy], dplyr [@dplyr], knitr [@knitr] are used. For the graphs, ggplot2 [@ggplot] is used. 
...
#| eval: true
#| label: fig-1
#| fig-cap: "Relationship between Free Will Belief and Job Satisfaction (Time 1)"
#| warning: false
```
...
#| eval: true
#| label: fig-2
#| fig-cap: "Relationship between Free Will Belief and Job Satisfaction (Time 2)"
#| warning: false
```
...
ggplot(data, aes(x = freewill, y = W2JS)) +
  ...
  theme_minimal()
```
Interpreting (@fig-1) and (@fig-2), which are scatter plots depicting the relationship between belief in free will and job satisfaction, we observe a clear positive correlation between the two variables. The upward trend in both graphs suggests that individuals who hold stronger convictions in free will tend to report higher levels of job satisfaction. This relationship is particularly evident in (@fig-2), where job satisfaction over time is plotted against belief in free will. The trendline in this graph shows a distinct positive slope, indicating that as belief in free will increases, so does job satisfaction. 
